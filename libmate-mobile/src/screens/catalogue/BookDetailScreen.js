...
export default function BookDetailScreen({ book, onClose }) {
  ...
  async function handleReserve() {
    ...
    try {
      ...
      Alert.alert('Reserved!', `"${fullBook.title}" is reserved for you. Collect it at the front desk within 48 hours.`);
    } catch (err) {
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <View style={styles.hero}>
          <Image
            source={fullBook.cover_image ? { uri: fullBook.cover_image } : PLACEHOLDER}
            style={styles.cover}
            resizeMode="cover"
          />
