...
import { getAllNewArrivals } from '@/api/trending';
...
function BookRow({ book, onPress }) {
  ...
  const daysAgo = book.days_since_added;
  const addedLabel = daysAgo === 0 ? 'Today' : daysAgo === 1 ? 'Yesterday' : `${daysAgo}d ago`;
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={styles.bottomRow}>
          ...
          <View style={styles.newBadge}>
            <Text style={styles.newBadgeText}>{addedLabel}</Text>
          </View>
        </View>
      </View>
...
export default function NewArrivalsScreen({ onClose }) {
  ...
  async function fetchPage(pageNum, replace = false) {
    try {
      const { data } = await getAllNewArrivals(pageNum, 20);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.topBarTitle}>New Arrivals</Text>
        <View style={{ width: 38 }} />
      </View>
...
const styles = StyleSheet.create({
  ...
  newBadge:  { backgroundColor: '#2C1F14', borderRadius: 20, paddingHorizontal: 8, paddingVertical: 2 },
  newBadgeText: { fontSize: 11, fontWeight: '700', color: '#FAF7F2' },
  separator: { height: 8 },
});
